...
exports.FileSystemTemplateLocator = function(options){
    ...
    var rootPath = fs.path(require.main).dirname() + "/views/";
...
exports.EjsLayoutRenderer = function(layout, request, templateLocator) {
    ...
    this.render = function(template, model, options) {
        ...
        var templateLocals = {
            script_tag: function(script) {
                return viewModel.start_tag_for("script", { src: "/public/js/" + script + ".js" }) + viewModel.tag_end("script");
            },
            ...
            partial : function(partialName) {
                ...
                {
                    partialName = partialName.substring(partialName.indexOf("/"));
                    isShared = true;
                }
            ...
            form_for: function(model, action, htmlOptions, callback){
                ...
                var tagName = function(name){
                    var prefix = htmlOptions.namespace || model.constructor.name.toLowerCase();
                    return prefix + "[" + name + "]";
                };
