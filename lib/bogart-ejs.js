...
exports.FileSystemTemplateLocator = function(options){
    ...
    this.getViewTemplate = function(relativePath){
        ...
        if (this.cacheViews && cache.view[relativePath]) {
            return cache.view[relativePath];
        }

        print("loading view from " + viewFilePath);
        if (fs.exists(viewFilePath)) {
            rawTemplate = fs.read(viewFilePath, "+r");
        } else if (fs.exists("_" + viewFilePath)) {
            rawTemplate = fs.read("_" + viewFilePath, "+r");
        }

        template = new EJS({ text: rawTemplate });
        cache.view[relativePath] = template;
    ...
    this.getLayoutTemplate = function(relativePath){
        ...
        if (this.cacheViews && cache.layout[relativePath]) {
            return cache.layout[relativePath];
        }
        ...
        template = new EJS({ text: rawTemplate });
        cache.layout[relativePath] = template;
    ...
    this.getSharedTemplate = function(relativePath) {
        ...
        if (this.cacheViews && cache.shared[relativePath]) {
            return cache.shared[relativePath];
        }
        ...
        template = new EJS({ text: rawTemplate });
        cache.shared[relativePath] = template;
    ...
    this.render = function(template, model, options) {
        ...
        var templateLocals = {
            script_tag: function(script) {
                return viewModel.start_tag_for("script", { src: "/js/" + script + ".js", type: "text/javascript" }) + viewModel.tag_end("script");
            },
