...
var appEvents = ["before_init","before_lookup_route","after_lookup_route","before_execute_route","after_execute_route"];
...
Bogart.App = function(options) {
    ...
    this.publish("before_init", this, options);
    ...
    this.handleRoute = function(verb, path, params) {
        ...
        this.publish("before_execute_route", routeContext);
...
print("mixed in observable");
require("./bogart/observable").mixin(Bogart.App.prototype);
print("Bogart.App.prototype.subscribers = " + JSON.stringify(Bogart.App.prototype.subscribers));

Bogart.App.subscribeTo = Bogart.App.prototype.subscribeTo;
Bogart.App.unsubscribe = Bogart.App.prototype.unsubscribe;
...
var RouteHandlerContext = exports.RouteHandlerContext = function(env, response, options) {
    ...
    this.templateLocator = this.getTemplateLocator(templateLocatorConfig);
...
RouteHandlerContext.prototype = {
    ...
    getTemplateLocator: function(config) {
        if (typeof this.templateLocators[config.viewsPath] === "undefined") {
            print("Instantiating locator for '" + config.viewsPath + "'");
            this.templateLocators[config.viewsPath] = new FileSystemTemplateLocator(config);
        }
        return this.templateLocators[config.viewsPath];
    },
    /**
     ...
     * @type Object
     */
    ejs: function(viewName, model, options){
        var renderOptions = {};
