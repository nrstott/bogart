...
App.prototype.start = function(port, host, jsgiOpts) {
  ...
  this.injector.factory('next', function () {
    return app;
  });

  this.injector.factory('nextApp', function () {
    return app;
  });

  this.middleware = this.middleware.reverse();
  first = this.middleware.shift();
  app = this.injector.invoke(first);

  if (this.middleware.length > 0) {
    this.middleware.forEach(function (middleware) {
      app = self.injector.invoke(middleware);
    });
  }
