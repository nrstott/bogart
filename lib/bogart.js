...
Bogart.App = function(appFn) {
    return new Bogart.App.fn.init(appFn);
};

var bogartAppMembers = {
    _routes: {},
    ...
    routerLoader: function(val) {
        if (val == undefined) {
            if (this._routerLoader == null) {
                var RouterLoader = require("./bogart/support/router_loader").RouterLoader;
                this._routerLoader = new RouterLoader();
            }
            return this._routerLoader;
        }

        this._routerLoader = val;
    },
     ...
     * @param resource
     */
    addRouter: function(path, resource){
        if (path == undefined || path == null || path == "")
            throw new Error("Argument 'path' is required");

        var self = this;

        var addRouterWithPath = function(path) {
            if (!path[0] == "/")
                path = "/" + path;
            ...
            self._resources.push({ path: path, resource: resource, paramNames: paramNames });
        };

        if (util.isArrayLike(path)) {
            for (var i=0;i<path.length;++i) {
                addRouterWithPath(path[i]);
            }
        } else {
            addRouterWithPath(path);
        }
    },
    ...
    run: function(env) {
        ...
        if (this.staticContent){
            ...
            return Jack.MethodOverride(Jack.Static(app, { urls: [this.staticContent] }))(env);
        } else {
            return Jack.MethodOverride(app)(env);
        }
    },
    ...
    handleRoute: function(verb, path, params) {
        ...
        for (var i=0;i<this._resources.length;++i){
            ...
            if (path.match(resourceRecord.path)) {
                pathParams = resourceRecord.path.exec(path);
                if (pathParams) {
                ...
                if (modifiedPath == "")
                    modifiedPath = "/";
                if (modifiedPath.length > 0 && modifiedPath.charAt(0) == "?")
        ...
        if (viewsRoot && !viewsRoot.match(/\/$/g))
            viewsRoot += "/";
        var result =  route.callback.apply(new RouteHandlerContext(self.env, self.response, {
            verb: verb,
            params: params,
            layout: self.layout,
            viewsRoot: viewsRoot
...
Bogart.App.fn = Bogart.App.prototype = {
    init: function(appFn) {
        util.update(this, util.deepCopy(bogartAppMembers));
        this.constructor = Bogart.App;

        addRouteMethods(this);

        var routerLoader = this.routerLoader();
        routerLoader.loadInto(this);
