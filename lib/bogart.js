...
RouteHandlerContext.prototype = {
    ...
    getTemplateLocator: function(config) {
        if (typeof this.templateLocators[config.viewPath] === "undefined") {
            print("Instantiating locator for '" + config.viewPath + "'");
            this.templateLocators[config.viewPath] = new FileSystemTemplateLocator(config);
        }
        return this.templateLocators[config.viewPath];
    },
    /**
