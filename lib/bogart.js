  ...
  fn = function(req) {
    ...
    try {
      ...
      return when(respPromise, function(resp) {
        if (resp === undefined) {
          return notFoundApp ? notFoundApp(req) : defaultNotFoundApp(req);
        }

        if (util.no(resp.status)) {
          return Q.reject('Response must have "status" property');
        }
        if (util.no(resp.body)) {
          return Q.reject('Response must have "body" property');
        }
        if (typeof resp.body.forEach !== 'function') {
          return Q.reject('Response "body" property must have a forEach method');
        }
 ...
 *
 * @returns {Promise | Undefined}  A promise that will resolve when 
...
exports.start = function(jsgiApp, options) {
  ...
  var http      = require('q-http')
    , server    = http.Server(jsgiApp)
    , host      = options.host || 'localhost'
    , port      = options.port || 8080
    , listening = server.listen(port, host);
  
  return when(listening, function() {
