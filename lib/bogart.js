...
exports.chain = function() {
  ...
  function middleware(req, next) {
    if (apps.length === 1) {
      console.log('calling chain 1');
      return apps[0](req, next);
    } else {
      console.log('calling chain');
      return exports.middleware.stack.apply(null, apps)(req, next);
    }
  }

  middleware.use = function(x) {
    apps.push(x);
  };
...
App.prototype.start = function(port, host) {
  ...
  if (this.middleware.length === 1) {
    app = this.middleware[0];
  } else {
    app = exports.middleware.stack.apply(null, this.middleware);
  }
...
exports.start = function(jsgiApp, options) {
  ...
  var bogartServer = require('bogart-server')
    , server = bogartServer(jsgiApp);

  return server.listen(options.port || 8080, options.host || options.hostname || 'localhost');
};
