...
var bogartBaseMembers = {
    ...
    addResource: function(resource){
        if (this._resources[resource.name])
            throw new Error("A resource for '" + resource.name + "' is already present");
    ...
    handleRoute: function(verb, path, params) {
        ...
        if (this._resources[resourceName]){
            ...
            var modifiedPath = path.substring(1 + resourceName.length);
            if (modifiedPath == "")
                modifiedPath = "/";

            var result = resource.handleRoute(verb, modifiedPath, {});
            if (result == undefined)
...
Bogart.Resource = function(name, routeFn){
    if (name == undefined || name == null || name == "" || name == "/")
        throw new Error("Resources must be named");
    this.name = name;
    ...
    var baseHandleRoute = this.handleRoute;
    this.handleRoute = function(verb, path, params) {
        if (!path.substring(0, name.length) == self.name)
            throw new Error("This path should not be routed to this resource: " + path);

        var modifiedPath = path.substring(name.length + 1, path.length);
        if (modifiedPath == "")
            modifiedPath = "/";
        
        return baseHandleRoute.call(self, verb, modifiedPath, params);
    };
