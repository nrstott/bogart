...
exports.FileSystemTemplateLocator = function(options){
    ...
    var rootPath;
    if (fs.isRelative(this.viewsPath)) {
        rootPath = fs.path(system.env["BOGART_ROOT"] || fs.join(fs.path(require.main).dirname(), this.viewsPath));
    } else {
        rootPath = this.viewsPath;
    }
    var cache = {
        layout: {},
        shared: {},
    ...
    var checkRelativePath = function(path){
        if (path === undefined || path === null) {
            throw new Error("Argument error, path is required");
        } else {
    ...
    this.getViewTemplate = function(path){
        ...
        log.debug("getViewTemplate::" + path);
        isRelative = checkRelativePath(path);

        if (isRelative) {
            if (/shared\//.test(path)){
                return this.getSharedTemplate(path.replace(/shared\//, ""));
            }
            viewFilePath = fs.path(fs.join(rootPath, path));
        } else {
            viewFilePath = path;
        }
        ...
        if (!fs.exists(viewFilePath)) {
            if (fs.exists("_" + viewFilePath)) {
                ...
                throw new Error("File not found: " + fs.absolute(viewFilePath));
            }
        }
    ...
    this.getLayoutTemplate = function(path){
        var template;
        var isRelative;
        var layoutFilePath;
        ...
        if (isRelative) {
            if (fs.isRelative(this.layoutsPath)) {
                layoutFilePath = fs.path(fs.join(rootPath, this.layoutsPath, path));
            } else {
                layoutFilePath = fs.path(fs.join(this.layoutsPath, path));
            }
        } else {
            layoutFilePath = path;
        }
    ...
    this.getSharedTemplate = function(path) {
        var sharedFilePath;
        var rawTemplate;
        var partialPath;
        var template;
        ...
        if (isRelative) {
            if (fs.isRelative(this.sharedPath)) {
                sharedFilePath = fs.path(fs.join(rootPath, this.sharedPath, path));
            } else {
                sharedFilePath = fs.path(fs.join(this.sharedPath, path));
            }
        } else {
            sharedFilePath = path;
        }
