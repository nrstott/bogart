...
exports.FileSystemTemplateLocator = function(options){
    ...
    this.cache_views = options.cache_views;
    if (this.cache_views === undefined || this.cache_views === null) this.cache_views = true;
    log.debug("cache_views = " + this.cache_views);
...
/**
 *
 * @param viewModel Instance of EjsView
 * @param log       Logger
 */
exports.BogartEjsView = function(viewModel, log) {
        ...
        form_for: function(model, action, htmlOptions, callback){
            ...
            var formHelper = {
                ...
                select: function(name, choices, htmlOptions){
                    setHtmlOptions(name, htmlOptions);
                    return viewModel.select_tag(tagName(name), viewModel.data[name] || "", choices, htmlOptions);
                },
            ...
            ["text_field","text_area","hidden_field","password_field"].forEach(function(helper){
                formHelper[helper] = function(name, htmlOptions){
                    ...
                    return viewModel[helper + "_tag"](tagName(name), viewModel.data[name] || "", htmlOptions);
                };
            });

            //___ejsO += viewModel.form_tag(action, htmlOptions);
            var formText = viewModel.form_tag(action, htmlOptions);
            ...
            if (method.toLowerCase() == "delete" || method.toLowerCase() == "put"){
                ...
                formText += viewModel.hidden_field_tag("_method", method);
            }

            var src = callback.toSource();
            src = callback.toSource().replace("{", "{ var ___ejsO = '';");
            var closingBrace = src.lastIndexOf("}");
            src = src.substring(0, closingBrace) + ";return ___ejsO;" + src.substring(closingBrace);

            // TODO: Find a better way to accomplish this, maybe edit the EJS module
            var scope = util.deepCopy(viewModel);
            util.update(scope, viewModel.data);
            with(scope) {
                callback = eval(src);
            }

            formText += callback(formHelper);
    ...
    util.update(this, this.helpers);
    util.update(this, viewModel.data);
...
exports.EjsLayoutRenderer = function(layout, templateLocator, options) {
    ...
    if (templateLocator === undefined || templateLocator === null)
        throw new Error("Argument required: '" + templateLocator + "'");
    ...
    this.render = function(template, model, options) {
        ...
        var helpers = exports.BogartEjsView(viewModel, log);

        helpers.content_for = function(namespace, callback) {
        ...
        helpers.partial = function(partialName, partialModel) {
            if (partialModel === undefined) {
                partialModel = viewModel.data;
            }
        ...
        publish(exports.EjsLayoutRenderer.Event.BEFORE_RENDER, viewModel, helpers, layoutLocals);

        with (helpers) {
            out = eval(template.out());

            if (options.useLayout == "no" || options.useLayout === false) {
                publish(exports.EjsLayoutRenderer.Event.AFTER_RENDER, out);
                return out;
            }
...
exports.EjsLayoutRenderer.Event = {
    BEFORE_RENDER: "before_render",
    AFTER_RENDER: "after_render"
};

require("bogart/observable").mixin(exports.EjsLayoutRenderer.prototype);
