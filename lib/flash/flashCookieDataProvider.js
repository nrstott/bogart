...
CookieDataProvider.prototype.previousFlash = function(req, flashId) {
  ...
  if (cookies && cookies[COOKIE_NAME]) {
    var hold = security.decrypt(decodeURIComponent(cookies[COOKIE_NAME]),
      this.options.encryptionKey || DEFAULT_ENCRYPTION_KEY);
...
CookieDataProvider.prototype.finalize = function(req, res, flashId) {
  ...
  var cookie = COOKIE_NAME + "=" +
    encodeURIComponent(security.encrypt(JSON.stringify(data),
      this.options.encryptionKey || DEFAULT_ENCRYPTION_KEY)) + ";Path=/;";
