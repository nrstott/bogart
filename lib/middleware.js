...
exports.Flash = function(options, nextApp) {
    options = options || {};
    options.flashIdProvider = options.flashIdProvider || new (require("./flash/flashIdCookieProvider"))({});
    options.flashDataProvider = options.flashDataProvider || new (require("./flash/flashCookieDataProvider"))({});
    ...
    return function(req) {
        ...
        req.env = req.env || {};
        var prevData = oldFlashId ? options.flashDataProvider.previousFlash(req, oldFlashId) : {};
        

        return when(prevData, function(data) {
            req.env.flash = data;
            // create the setter for new flash data
            var setter = options.flashDataProvider.setter(req, newFlashId);
