...
middleware.freshEnv = middleware(function (req, next) {
 ...
 *
 */
middleware.oauth = require('./middleware/oauth');
...
middleware.twitter = require('./middleware/twitter');
 ...
 * Assigns named params to req.env.domain. Only :named paramters are supported, 
 * no wildcards or regular expressions. 
 *
 * If a non-standard port is present in the hostname it will always be assigned to
 * req.env.domain.port, and does not need to be handled explicitly.
 * 
 * Example: 
 *    app.use(bogart.middleware.domainFilter, ":tenant.github.com")
 *
 * @param {String} domainPatter the pattern to execute against the domain/hostname
 * @param {Function} nextApp  The next application in the JSGI chain.
 * @returns {Function} JSGI Application.
 */
middleware.domainFilter = require('./middleware/domainFilter');
...
middleware.batteries = function(config, nextApp) {
  ...
  var stack = middleware.freshEnv(
    middleware.error(config.error,
      middleware.gzip(
