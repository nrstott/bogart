...
exports.directory = exports.Directory = function(root, nextApp) {
    ...
    return function(req) {
        ...
        return stat(reqPath).then(function(stats) {
            if (stats.isFile()) {
                return respondWithFile(req, reqPath, opts.headers);
            } else if (stats.isDirectory()) {
                var indexPath = path.join(reqPath, "index.html");
                return stat(indexPath).then(function(stats) {
                    if (stats.isFile()) {
                        return respondWithFile(req, indexPath, opts.headers);
            } else {
                        throw false;
            }
                });
...
exports.batteries = function(config, nextApp) {
    ...
    config.root = config.root || 'public';
