...
function join(forEachable) {
  ...
  return when(forEachable, function(forEachable) {
...
exports.Gzip = function(nextApp) {
  ...
  function acceptsGzip(req) {
    ...
    return encodings.filter(function(x) { return x === 'gzip'; }).length > 0;
  }
  ...
  return function(req) {
    ...
    if (acceptsGzip(req)) {
      return Q.when(resp, function(resp) {
        var ds = deflate.createDeflateStream(new ForEachStream(resp.body.map(function(chunk) { return new Buffer(chunk); })));
