 ...
 * @returns {Function} JSGI Middleware
 */
exports.parseForm = exports.ParseForm = function() {
  return function(req, nextApp) {
    var contentType = req.headers['content-type'];
...
exports.gzip = exports.Gzip = function(opts) {
  opts = opts || {};
  ...
  function middleware(req, nextApp) {
    var zlib   = require('zlib')
      , bogart = require('./bogart')
      , resp;
    
    if (typeof nextApp !== 'function') {
      throw new Error('Expected a function for the second parameter to error middleware');
    }
 ...
 */
exports.methodOverride = exports.MethodOverride = function(opts) {
  opts = opts || {};

  return function(request, nextApp) {
    try {
...
/**
 * JSGI B Shim around the Parted middleware for Node.JS
 * https://github.com/chjj/parted
 *
 * @param {Object} opts Options for Parted.
 * @returns {Function} JSGI Middleware.
 */
exports.parted = exports.Parted = function(opts) {
  opts = opts || {};

  if (arguments.length < 2) {
    return middleware;
  } else {
    return middleware(arguments[0], arguments[1]);
  }
 ...
 */
exports.error = exports.Error = function(errorResponse) {
  ...
  return function(req, next) {
    var res = Q.call(function() {
      return next(req);
    });

    return Q.when(res, function(val) { 
      return val;
    }, function(err) {
 ...
 * @returns {Function} JSGI response.
 */
exports.validateResponse = exports.ValidateResponse = function(req, nextApp) {
  var res = Q.call(function() {
    return nextApp(req);
  });

  return Q.when(res, function(resp) {
    function reject(msg) {
      var defer = Q.defer();
...
exports.directory = exports.Directory = function(root) {
  var req, nextApp
  ...
  if (arguments.length === 2) {
    ...
    return responder(req, nextApp);
  } else {
    return responder;
  }

  function responder(req, nextApp) {
    var opts;
    ...
    var reqPath = path.join(opts.root, req.pathname.substring(1));
...
exports.flash = exports.Flash = function(config) {
  config = config || {};
  ...
  return function(req, nextApp) {
    ...
    return when(prevData, function(data) {
      ...
      var setter = config.flashDataProvider.setter(req, newFlashId).bind(config.flashDataProvider);
 ...
 *
 */
exports.oauth2 = function(config) {
  ...
  return function(req, nextApp) {
    ...
    function authd(req) {
      if (!req.session('profile')) {
        return bogart.redirect(options.loginRoute + '?returnUrl=' + req.pathname);
      }
      req.auth = req.auth || {};
    ...
    return exports.stack(router, authd)(req);
  }
}
...
exports.facebook = function(config) {
  ...
  var oauth2 = exports.oauth2(facebookStrategy);
    
  return function(req, nextApp) {
    return oauth2(req, nextApp);
  };
}
 ...
 * @returns {Function} JSGI Application.
 */
exports.stringReturnAdapter = function() {
  return function(req, nextApp) {
...
/**
 * Construct a branch, a binary split middleware.
 *
 * Logically, this middleware has two Bogart Chains: `ifTrue` and `ifFalse`.
 * There are two methods on the object returned by this function, `ifTrue` and `ifFalse` that
 * function like a `use` for a Bogart Chain.
 *
 * If `conditional` returns `true`, then `ifTrue` chain is executed. Otherwise, `ifFalse` is executed.
 *
 * Example:
 * 
 *     var branch = bogart.middleware.branch(function(req) { return req.method === 'GET'; });
 *     splitter(req, function(req) {
 *       return bogart.text('is get request');
 *     }, function(req) {
 *       return bogart.text('is not get request');
 *     });
 *
 * @param {Function} conditional A function that takes a JSGI request and returns a boolean.
 * @returns {Function} JSGI Middleware that takes three arguments: a JSGI request and two JSGI Middleware.
 */
exports.branch = function(conditional) {
  var bogart = require('./bogart')
    , ifTrue = bogart.chain()
    , ifFalse = bogart.chain();

  middleware.ifTrue = function(mw) {
    ifTrue.use(mw);
  };

  middleware.ifFalse = function(mw) {
    ifFalse.use(mw);
  };
  ...
  function middleware(req) {
    var match = conditional;

    if (typeof match === 'function') {
      match = match(req);
    }

    return match ? ifTrue(req) : ifFalse(req);
  }
};
 ...
 */
exports.cascade = function(accept) {

  return function(req /*, apps */) {
    var deferred = Q.defer()
      , args = Array.prototype.slice.call(arguments)
      , apps = [];
 ...
 */
exports.batteries = function(config) {
  config = config || {};
  ...
  function middleware(req, nextApp) {
    ...
    return stack(exports.error(), exports.gzip(), exports.directory(root), exports.parted(), exports.methodOverride(),
      exports.session(), exports.flash(), exports.bodyAdapter(), exports.stringReturnAdapter(), nextApp)(req);
  };
};
...
function stack(/* apps... */) {
  ...
  return function(req) {
    ...
    function next(req) {
      var jsgiApp = middleware.shift();

      if (!jsgiApp) {
        return Q.reject(new Error('Empty chain'));
      }
