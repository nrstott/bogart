...
/**
 * Adds route style parameter extraction to the domain/hostname portion of the URI.
 * Assigns named params to req.body. Only :named paramters are supported, 
 * no wildcards or regular expressions. 
 *
 * If an explicit port is present in the hostname it will always be assigned to
 * req.body.port, and does not need to be handled explicitly.
 * 
 * Example: 
 *    app.use(bogart.middleware.domainParameters, ":tenant.github.com")
 *
 * @param {String} domainPatter the pattern to execute against the domain/hostname
 * @returns {Function} JSGI middleware.
 */
middleware.domainParameters = require('./middleware/domainParameters');
...
batteriesConfig.default = {
  directory: 'public',
  session: {},
  error: { showError: false, logError: true }
};
 ...
 */
middleware.batteries = function(app, config) {
  config = batteriesConfig(config);
  if (config.secret) {
    config.session.secret = config.secret;
  }

  app.use(middleware.error(config.error));
  app.use(middleware.gzip());
  app.use(middleware.parted(config.parted || undefined));
  app.use(middleware.methodOverride());
  app.use(middleware.session(config.session || undefined));
  app.use(middleware.flash(config.flash || undefined));
  app.use(middleware.bodyAdapter);
  app.use(middleware.stringReturnAdapter());
