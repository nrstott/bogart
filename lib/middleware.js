...
exports.Error = function(app, errorResponse) {
  errorResponse = errorResponse || function(err) {
    return {
      status: 500,
      body: [ '<html><head><title>Error</title></head><body>An error occurred.', 
        err.message ? err.message : '', '<br /><br />', 
        err.stack ? err.stack : '', '</body>' ],
...
exports.oauth2 = function(config, nextApp) {
  var OAuth2 = new (require('oauth').OAuth2)(config.clientId, config.clientSecret,
                                            '', config.authorizationURL, config.tokenURL)
      , bogart = require('./bogart')
      , redirectUri = 'http://localhost:8085/facebookCallback';


  var router = bogart.router(null, nextApp);

  nextApp.before(function(req) {
     if(!req.session('facebook_profile')) {
         bogart.redirect('/facebookLogin');
     }
  });

  router.get('/facebookLogin', function(req) {
    var url = OAuth2.getAuthorizeUrl({ response_type: 'code', redirect_uri: redirectUri});
    
    return bogart.redirect(url);
  });

  router.get('/facebookLogout', function(req) {
      req.session('facebook_profile',undefined);
  ...
  router.get('/facebookCallback', function(req) {
    ...
    if (queryString && queryString.code) {
      ...
      OAuth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: redirectUri },
        function(err, accessToken, refreshToken) {
          OAuth2.getProtectedResource('https://graph.facebook.com/me', accessToken, function (err, body, res) {                        
            try {
              o = JSON.parse(body);
              ...
              deffered.resolve(bogart.redirect('/profile'));
            } catch(e) {
              deffered.reject(e);
            }
          });
