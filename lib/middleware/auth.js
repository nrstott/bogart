...
Strategy.prototype.execute = function () {
  ...
  q.when(this.authenticate(), function (user) {
...
function Auth(config) {
  ...
  router.get(config.loginUrl, function (req) {
    strategy.login();
  });

  router.get(config.logoutUrl, function (req) {

  });

  var auth = function (nextApp) {
    return router()(nextApp);
  };

  var strategies = {};

  auth.addStrategy = function (strategy) {
    if (!strategy.name) {
