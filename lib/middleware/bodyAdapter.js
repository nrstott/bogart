...
module.exports = middleware(function (req, nextApp) {
  return q.when(nextApp(req), function (resp) {
    ...
    var reason = 'Middleware error: The response object returned by your route handler for "'+req.pathInfo+'" is not a proper bogart JSGI response.  It does not have a forEach method, is not a Buffer, and is not a readable Stream';
