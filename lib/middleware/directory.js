...
function respondWithFile(req, root, filePath, contentType, headers) {
  if (!qfs.contains(root, filePath)) {
    return response404;
  }
  return q.when(fsp.stat(filePath), function (stat) {
    ...
    if (!stat.isFile()) {
      if (stat.isDirectory()) {
        ...
        return respondWithFile(req, root, indexPath, contentType, headers);
      } else {
        return response404;
      }
    }
...
module.exports = function directory(root, nextApp) {
  ...
  return function (req) {
    ...
    return q.when(respondWithFile(req, root, reqPath, opts.headers), function(res) {
