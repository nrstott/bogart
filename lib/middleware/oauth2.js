...
module.exports = function oauth2Middleware(config) {
  ...
  var authd = function(req, nextApp) {
      ...
      OAuth2.getOAuthAccessToken(code, {
        ...
        OAuth2.getProtectedResource(options.resourceURL, accessToken, function(err, body, res) {
          try {
            ...
            var profile = q(options.parse(body)).then(function (profile) {
            ...
            deferred.resolve(bogart.redirect(req.params.returnUrl || 
              (req.session && req.session('returnUrl')) || 
              '/'));
          } catch (e) {
            deferred.reject(e);
          }
