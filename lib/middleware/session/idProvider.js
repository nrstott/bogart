...
IdProvider.prototype.save = function(req, res, sessionId) {
    return setCookie(req, res, sessionId, this.config.lifetime, this.config.secret);
};

IdProvider.prototype.destroy = function (req, res, sessionId) {
    return setCookie(req, res, sessionId, -1, this.config.secret);
};

function setCookie(req, res, sessionId, expiresInSeconds, secret) {
    res = res || {};
...
function isNotSessionIdCookie(el) {
    return !(new RegExp("^" + this.cookieKey + "=").test(el));
}

function sessionCookie(sessionId, secret, expires) {
