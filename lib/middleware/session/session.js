...
exports.Session = function (config) {
  config = config || {};
  ...
  if (config.secret) {
    config.options.idProvider.secret = config.secret;
  ...
  return function (req, next) {
    ...
    return when(dataProvider.loadSession(req, sessionId), function (session) {
      ...
      req.session.destroy = function () {
        req.session._destroy = true;
      };

      return when(next(req), function (res) {
        if (req.session._destroy === true) {
          res = idProvider.destroy(req, res, sessionId) || res;
          return when(dataProvider.destroy(req, res, sessionId), function (dpResp) {
            return dpResp || res;
          });
        } else {
