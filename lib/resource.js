...
function Resource(name, viewEngine) {
  ...
  this.router.get(this.routePrefix+'/new', function (req) {
    ...
    return q(res).then(function (res) {
      console.log('res', res);
      res.link = res.link || {};
      res.link.create = self.routePrefix+'/create';
      return self.respond(req, res);
    });
  });
  ...
  this.router.post(this.routePrefix+'/create', function (req) {
    var res = self.create(req.params);

    return q(res).then(function (id) {
      return bogart.redirect(self.showUrl(id));
    });
  });

  this.router.get(this.routePrefix+'/:id', function (req) {
    ...
    return function (res) {
      ...
      if (res.id) {
        defaultLinks = _.extend(defaultLinks, {
          update: self.updateUrl(res.id),
          destroy: self.destroyUrl(res.id),
          show: self.showUrl(res.id)
        });
      }
