...
DataProvider.prototype.loadSession = function(req, sessionId) {
    ...
    req.env = req.env || {};
    req.env.session = cookie ? JSON.parse(security.decrypt(decodeURIComponent(cookie), this.config.encryptionKey)) : {};

    // set up the session function
    req.session = function() {
        var args = Array.prototype.slice.call(arguments);
        if (args.length === 2) {
            req.env.session[args[0]] = args[1];
        } else {
            return req.env.session[args[0]];
        }
    };

    req.session.keys = function() {
        return Object.keys(req.env.session);
    };
};
